...
public class LoginServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        req.setAttribute("inc","0");
        req.setAttribute("reg","0");
        req.getServletContext().getRequestDispatcher("/jsp/login_register.jsp").forward(req, resp);
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(users.isEmpty()){
            req.setAttribute("inc","1");
            req.setAttribute("reg","0");
            req.getServletContext().getRequestDispatcher("/jsp/login_register.jsp").forward(req, resp);
        }else{
            for (User item : users) {
                if(item.getFirstName().equals(name) && item.getPassword().equals(password)){
                    HttpSession session = req.getSession();
                    session.setAttribute("user", name);
                    req.setAttribute("nick",name);
